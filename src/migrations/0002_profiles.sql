...
CREATE TABLE "user_exam_targets" (
	"id" bigint PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY (sequence name "user_exam_targets_id_seq" INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 START WITH 1 CACHE 1),
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "user_exam_targets_user_id_exam_id_key" UNIQUE("user_id","exam_id")
);
--> statement-breakpoint
ALTER TABLE "user_exam_targets" ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
CREATE INDEX "user_exam_targets_user_id_idx" ON "user_exam_targets" USING btree ("user_id");--> statement-breakpoint
...
-- The catalogue is public; a profile is not. Their bio, institution and target
-- have no signed-out reader.
...
-- The five exams the question bank actually holds papers for. bank and role are
-- spelled exactly as the question JSON spells them, because that pair is the
-- join key -- any other spelling matches no questions.
--
...
INSERT INTO public.exams (slug, bank, role, display_name, country_code, sort_order) VALUES
  ('ibps-po',    'IBPS', 'PO',    'IBPS PO',    'IN', 1),
  ('sbi-clerk',  'SBI',  'Clerk', 'SBI Clerk',  'IN', 2),
  ('ibps-rrb',   'IBPS', 'RRB',   'IBPS RRB',   'IN', 3),
  ('sbi-po',     'SBI',  'PO',    'SBI PO',     'IN', 4),
  ('ibps-clerk', 'IBPS', 'Clerk', 'IBPS Clerk', 'IN', 5)
